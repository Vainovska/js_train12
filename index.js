...
function createDictionary(entries) {
  ...
  entries.forEach((value, key) => [value, key]);
...
function addKeyValuePairs(dictionary, entries) {
  ...
  const obj = Object.fromEntries(dictionary, added, rejected);
  return obj;
...
function checkPresence(dictionary, keys) {
  ...
  dictionary.forEach((value, key) =>
    keys.includes(key) ? arr.push("true") : arr.push("false")
  );
...
function sortByValues(dictionary) {
  ...
  const dict = new Map(rev.map((key, value) => key));
  return dict;
